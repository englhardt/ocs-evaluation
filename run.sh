...
julia --project scripts/precompute_parameters.jl $(pwd)/scripts/config/config_precompute_parameters.jl | tee logs/$SCENARIO/precompute_params.log
julia --project scripts/generate_experiments.jl $(pwd)/scripts/config/config_warmup.jl 2>&1 | tee logs/$SCENARIO/generate_warmup.log
...
mkdir -p logs/$SCENARIO
julia --project scripts/precompute_parameters_gt.jl $(pwd)/scripts/config/config_precompute_parameters_gt.jl | tee logs/$SCENARIO/precompute_params.log
julia --project scripts/generate_experiments.jl $(pwd)/scripts/config/config_$SCENARIO.jl 2>&1 | tee logs/$SCENARIO/generate.log
